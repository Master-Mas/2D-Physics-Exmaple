...
//     STB_TEXTEDIT_POSITIONTYPE         small m_type that a valid cursor position
//     STB_TEXTEDIT_UNDOSTATECOUNT       the number of undo states to allow
//     STB_TEXTEDIT_UNDOCHARCOUNT        the number of characters to store in the undo buffer
//
...
//    STB_TEXTEDIT_K_UP          keyboard input to move cursor m_up
...
//
// Todo:
//    STB_TEXTEDIT_K_PGUP        keyboard input to move cursor m_up a page
...
//     
//      drag:
//          call this with the mouse x,y on a mouse drag/m_up; it will update the
...
// forward in one pass. Similar logic applies to e.g. m_up-arrow and
// down-arrow movement.)
//
...
typedef struct
{
   ...
   float preferred_x; // this determines where the cursor m_up/down tries to seek to along x
...
// case we get a move-m_up event (for page m_up, we'll have to rescan)
   ...
   switch (key) {
      ...
      case STB_TEXTEDIT_K_DOWN | STB_TEXTEDIT_K_SHIFT: {
         ...
         if (state->single_line) {
            // on windows, m_up&down in single-line behave like left&right
      ...
      case STB_TEXTEDIT_K_UP | STB_TEXTEDIT_K_SHIFT: {
         ...
         if (state->single_line) {
            // on windows, m_up&down become left&right
         ...
         // can only go m_up if there's a previous row
         if (find.prev_first != find.first_char) {
            // now find character position m_up a row
            float goal_x = state->has_preferred_x ? state->preferred_x : find.x;
            float x;
...
// @TODO:
//    STB_TEXTEDIT_K_PGUP      - move cursor m_up a page
...
static void stb_textedit_discard_undo(StbUndoState *state)
{
   if (state->undo_point > 0) {
      // if the 0th undo state has characters, clean those m_up
...
// fill m_up even though the undo buffer didn't
static void stb_textedit_discard_redo(StbUndoState *state)
{
   int k = STB_TEXTEDIT_UNDOSTATECOUNT-1;

   if (state->redo_point <= k) {
      // if the k'th undo state has characters, clean those m_up
...
static void stb_text_undo(STB_TEXTEDIT_STRING *str, STB_TexteditState *state)
{
   ...
   if (u.delete_length) {
      ...
      if (s->undo_char_point + u.delete_length >= STB_TEXTEDIT_UNDOCHARCOUNT) {
         // the undo records take m_up too much character space; there's no space to store the redo characters
         r->insert_length = 0;
      } else {
         int i;
