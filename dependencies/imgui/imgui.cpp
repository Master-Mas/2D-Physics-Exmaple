...
/*
 ...
 A: Overwrite the following files:
    ...
    Check the "API BREAKING CHANGES" sections for a list of occasional API breaking changes. If you have a problem with a function, search for its m_name
    in the code, there will likely be a comment about it. Please report any issue to the GitHub page!


 Q: What is ImTextureID and how do I display an image?
 A: ImTextureID is a void* used to pass renderer-agnostic texture references around until it hits your render function.
    ImGui knows nothing about what those bits represent, it just passes them around. It is m_up to you to decide what you want the void* to carry!
 ...
 A: Yes. A primer on the use of labels/IDs in ImGui..
   ...
   - When working with trees, ID are used to preserve the open/close state of each tree node.
     Depending on your use cases you may want to use strings, m_indices or pointers as ID.
      e.g. when displaying a single object that may change over time (1-1 relationship), using a static string as ID will preserve your node open/closed state when the targeted object change.
      e.g. when displaying a list of objects, using m_indices or pointers as ID will preserve the node open/closed state differently. experiment and see what makes more sense!
 ...
 - tip: you can call Begin() multiple times with the same m_name during the same frame, it will keep appending to the same window. this is also useful to set yourself in the context of another window (to get/set other settings)
 ...
 - window: add a way for very transient windows (non-saved, temporary overlay over hundreds of objects) to "clean" m_up from the global window list. perhaps a lightweight explicit cleanup pass.
...
!- scrolling: allow immediately effective change of scroll if we haven't appended items yet
 ...
 - widgets: clean m_up widgets internal toward exposing everything.
...
!- main: make it so that a frame with no window registered won't refocus every window on subsequent frames (~bump LastFrameActive of all windows).
 - main: IsItemHovered() make it more consistent for various m_type of widgets, widgets with multiple components, etc. also effectively IsHovered() region sometimes differs from hot region, e.g tree nodes
 - main: IsItemHovered() info stored in a stack? so that 'if TreeNode() { Text; TreePop; } if IsHovered' return the hover state of the TreeNode?
 - input text: clean m_up the mess caused by converting UTF-8 <> wchar. the code is rather inefficient right now.
 ...
 - input text multi-line: way to dynamically grow the buffer without forcing the user to initially allocate for worse case (follow m_up on #200)
 - input text multi-line: line numbers? status bar? (follow m_up on #200)
 - input text multi-line: behave better when user changes input buffer while editing is active (even though it is illegal behavior). namely, the change of buffer can create a scrollbar glitch (#725)
 - input text: allow centering/positioning text so that ctrl+clicking Drag or Slider keeps the textual value at the same pixel position.
 - input number: optional range min/max for Input*() functions
 - input number: holding [-]/[+] buttons could increase the step speed non-linearly (or user-controlled)
 - input number: use mouse wheel to step m_up/down
 - input number: applying arithmetics ops (+,-,*,/) messes m_up with text edit undo stack.
 ...
 - layout: clean m_up the InputFloatN/SliderFloatN/ColorEdit4 layout code. item width should include frame padding.
...
!- popups/menus: clarify usage of popups id, how MenuItem/Selectable closing parent popups affects the ID, etc. this is quite fishy needs improvement! (#331, #402)
 ...
 - menus: calling BeginMenu() twice with a same m_name doesn't seem to append nicely
...
!- color: the color helpers/typing is a mess and needs sorting out.
 ...
 - plot: add a helper e.g. Plot(char* label, float value, float time_span=2.0f) that stores values and Plot them for you - probably another function m_name. and/or automatically allow to plot ANY displayed value (more reliance on stable ID)
 - slider: allow using the [-]/[+] buttons used by InputFloat()/InputInt()
 - slider: initial absolute click is imprecise. change to relative movement slider (same as scrollbar).
 - slider: add dragging-based widgets to edit values with mouse (on 2 axises), saving screen real-estate.
 - slider: tint background based on value (e.g. v_min -> v_max, or use 0.0f either side of the sign)
 - slider & drag: int data passing through a float
 - drag float: m_up/down axis
 ...
 - style/opt: PopStyleVar could be optimized by having GetStyleVar returns the m_type, using a table mapping stylevar enum to data m_type.
...
!- keyboard: tooltip & combo boxes are messing m_up / not honoring keyboard tabbing
...
static ImGuiWindow*     CreateNewWindow(const char* m_name, ImVec2 size, ImGuiWindowFlags flags);
...
static ImGuiIniData*    FindWindowSettings(const char* m_name);
static ImGuiIniData*    AddWindowSettings(const char* m_name);
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.PrevLineHeight = (line_height - GImGui->Style.ItemSpacing.y);    // If we end m_up needing more accurate data (to e.g. use SameLine) we may as well make the clipper have a fourth step to let user process and display the last item in their list.
...
ImGuiWindow::ImGuiWindow(const char* m_name)
{
...
// Note that we still point to some static data and members (such as GFontAtlas), so the state instance you end m_up using will point to the static data within its module
...
static ImGuiIniData* AddWindowSettings(const char* m_name)
{
    ...
    ini->Name = ImStrdup(m_name);
    ini->Id = ImHash(m_name, 0);
    ini->Collapsed = false;
...
static void LoadSettings()
{
    ...
    for (const char* line_start = file_data; line_start < buf_end; )
    {
        ...
        {
            char m_name[64];
            ImFormatString(m_name, IM_ARRAYSIZE(m_name), "%.*s", (int)(line_end-line_start-2), line_start+1);
            settings = FindWindowSettings(m_name);
            if (!settings)
                settings = AddWindowSettings(m_name);
        }
        else if (settings)
        {
            float x, y;
...
static void SaveSettings()
{
    ...
    for (int i = 0; i != g.Settings.Size; i++)
    {
        ...
        if (settings->Pos.x == FLT_MAX)
            continue;
        const char* m_name = settings->Name;
        if (const char* p = strstr(m_name, "###"))  // Skip to the "###" marker if any. We don't skip past to match the behavior of GetID()
            m_name = p;
        fprintf(f, "[%s]\n", m_name);
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list)
{
    ...
    // Check that draw_list doesn't use more m_vertices than indexable (default ImDrawIdx = 2 bytes = 64K m_vertices)
    // If this assert triggers because you are drawing lots of stuff manually, A) workaround by calling BeginChild()/EndChild() to put your draw commands in multiple draw lists, B) #define ImDrawIdx to a 'unsigned int' in imconfig.h and render accordingly.
    IM_ASSERT((int64_t)draw_list->_VtxCurrentIdx <= ((int64_t)1L << (sizeof(ImDrawIdx)*8)));  // Too many m_vertices in same ImDrawList. See comment above.
...
static bool BeginPopupEx(const char* str_id, ImGuiWindowFlags extra_flags)
{
    ...
    if (flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(m_name, 20, "##menu_%d", g.CurrentPopupStack.Size);    // Recycle windows based on depth
    else
        ImFormatString(m_name, 20, "##popup_%08x", id); // Not recycling, so we can close/open during the same frame

    bool is_open = ImGui::Begin(m_name, NULL, flags);
...
bool ImGui::BeginPopupModal(const char* m_name, bool* p_open, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    bool is_open = ImGui::Begin(m_name, p_open, flags);
...
static ImVec2 FindBestPopupWindowPos(const ImVec2& base_pos, const ImVec2& size, int* last_dir, const ImRect& r_inner)
{
    ...
    for (int n = (*last_dir != -1) ? -1 : 0; n < 4; n++)   // Last, Right, down, m_up, left. (Favor last used direction).
    {
...
static ImGuiWindow* CreateNewWindow(const char* m_name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    IM_PLACEMENT_NEW(window) ImGuiWindow(m_name);
    window->Flags = flags;
    ...
    else
    {
        ...
        ImGuiIniData* settings = FindWindowSettings(m_name);
        if (!settings)
        {
            settings = AddWindowSettings(m_name);
        }
...
// - Begin/End can be called multiple times during the frame with the same window m_name to append content.
...
// - The window m_name is used as a unique identifier to preserve window information across frames (and save rudimentary information to the .ini file).
...
{
    return ImGui::Begin(m_name, p_open, ImVec2(0.f, 0.f), -1.0f, flags);
}

bool ImGui::Begin(const char* m_name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(m_name != NULL);                        // Window m_name required
    IM_ASSERT(g.Initialized);                       // Forgot to call ImGui::NewFrame()
    ...
    if (!window)
    {
        window = CreateNewWindow(m_name, size_on_first_use, flags);
        window_is_new = true;
    }
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if ((flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup))
        {
            ...
            window->Size = window->SizeFull = size_on_first_use; // NB: argument m_name 'size_on_first_use' misleading here, it's really just 'size' as provided by user passed via BeginChild()->Begin().
        }
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            const ImVec2 text_size = CalcTextSize(m_name, NULL, true);
            ...
            RenderTextClipped(text_min, text_max, m_name, NULL, &text_size, style.WindowTitleAlign, &clip_min, &clip_max);
        }
    ...
    // We set this m_up after processing the resize grip so that our clip rectangle doesn't lag by a frame
    // Note that if our window is collapsed we will end m_up with a null clipping rectangle which is the correct behavior.
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
    ...
    if (held && grab_h_norm < 1.0f)
    {
        ...
        // Apply scroll
        // It is ok to modify Scroll here because we are being called in Begin() after the calculation of SizeContents and before setting m_up our starting position
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, float val)
{
    ...
    IM_ASSERT(pvar != NULL); // Called function with wrong-m_type? Variable is not a float.
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, const ImVec2& val)
{
    ...
    IM_ASSERT(pvar != NULL); // Called function with wrong-m_type? Variable is not a ImVec2.
    ImGuiStyleMod backup;
    backup.Var = idx;
...
void ImGui::SetWindowPos(const char* m_name, const ImVec2& pos, ImGuiSetCond cond)
{
    ImGuiWindow* window = FindWindowByName(m_name);
    if (window)
        SetWindowPos(window, pos, cond);
...
void ImGui::SetWindowSize(const char* m_name, const ImVec2& size, ImGuiSetCond cond)
{
    ImGuiWindow* window = FindWindowByName(m_name);
    if (window)
...
void ImGui::SetWindowCollapsed(const char* m_name, bool collapsed, ImGuiSetCond cond)
{
    ImGuiWindow* window = FindWindowByName(m_name);
    if (window)
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            // Relies on repeat logic of IsMouseClicked() but we may as well do it ourselves if we end m_up exposing finer RepeatDelay/RepeatRate settings.
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float m_radius)
{
    ImGuiWindow* window = GetCurrentWindow();

    const ImRect bb(pos - ImVec2(m_radius,m_radius), pos + ImVec2(m_radius,m_radius));
    ...
    window->DrawList->AddCircleFilled(center, ImMax(2.0f, m_radius), col, 12);

    const float cross_extent = (m_radius * 0.7071f) - 1.0f;
    if (hovered)
    {
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We vertically grow m_up to current line height m_up the typical widget height.
    ...
    // (Ideally we'd want to add a flag for the user to specify we want want the hit test to be done m_up to the right side of the content or not)
...
float ImGui::RoundScalar(float value, int decimal_precision)
{
    ...
    // So when our value is 1.99999 with a precision of 0.001 we'll end m_up rounding to 2.0
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const float m_radius = check_bb.GetHeight() * 0.5f;

    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, id, &hovered, &held);

    window->DrawList->AddCircleFilled(center, m_radius, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if (active)
    {
        const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
        window->DrawList->AddCircleFilled(center, m_radius-pad, GetColorU32(ImGuiCol_CheckMark), 16);
    }

    if (window->Flags & ImGuiWindowFlags_ShowBorders)
    {
        window->DrawList->AddCircle(center+ImVec2(1,1), m_radius, GetColorU32(ImGuiCol_BorderShadow), 16);
        window->DrawList->AddCircle(center, m_radius, GetColorU32(ImGuiCol_Border), 16);
    }
...
#define STB_TEXTEDIT_K_UP           0x10002 // keyboard input to move cursor m_up
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together (they both use m_up/down keys)
...
{
    // Hexadecimal input provided as a convenience but the flag m_name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/m_up without looking at scrollbar.
...
void ImGui::Columns(int columns_count, const char* id, bool border)
{
    ...
    // Differentiate column ID with an arbitrary prefix for cases where users m_name their columns set the same as another widget.
        ...
        struct Funcs
        {
            static void NodeDrawList(ImDrawList* draw_list, const char* label)
            {
                ...
                for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
                {
                    ...
                    if (!pcmd_node_open)
                        continue;
                    ImGuiListClipper clipper(pcmd->ElemCount/3); // Manually coarse clip our print out of individual m_vertices to save CPU, only items that may be visible.
                    while (clipper.Step())
