...
namespace ImGui
{
    ...
    // Window
    IMGUI_API bool          Begin(const char* m_name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);                                                   // push window to the stack and start appending to it. see .cpp for details. return false when window is collapsed, so you can early out in your code. 'bool* p_open' creates a widget on the upper-right to close the window (which sets your bool to false).
    IMGUI_API bool          Begin(const char* m_name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha = -1.0f, ImGuiWindowFlags flags = 0); // OBSOLETE. this is the older/longer API. the extra parameters aren't very relevant. call SetNextWindowSize() instead if you want to set a window size. For regular windows, 'size_on_first_use' only applies to the first time EVER the window is created and probably not what you want! might obsolete this API eventually.
    ...
    IMGUI_API void          SetWindowPos(const char* m_name, const ImVec2& pos, ImGuiSetCond cond = 0);      // set named window position.
    IMGUI_API void          SetWindowSize(const char* m_name, const ImVec2& size, ImGuiSetCond cond = 0);    // set named window size. set axis to 0.0f to force an auto-fit on this axis.
    IMGUI_API void          SetWindowCollapsed(const char* m_name, bool collapsed, ImGuiSetCond cond = 0);   // set named window collapsed state
    IMGUI_API void          SetWindowFocus(const char* m_name);                                              // set named window to be focused / front-most. use NULL to remove focus.
    ...
    // Widgets: Value() Helpers. Output single value in "m_name: value" format (tip: freely declare more in your code to handle your types. you can add functions to the ImGui namespace)
    ...
    IMGUI_API bool          BeginPopupModal(const char* m_name, bool* p_open = NULL, ImGuiWindowFlags extra_flags = 0);               // modal dialog (can't close them by clicking outside)
    ...
    IMGUI_API bool          IsKeyDown(int key_index);                                           // key_index into the keys_down[] array, imgui doesn't know the semantic of each entry, uses your own m_indices!
    IMGUI_API bool          IsKeyPressed(int key_index, bool repeat = true);                    // uses user's key m_indices as stored in the keys_down[] array. if repeat=true. uses io.KeyRepeatDelay / KeyRepeatRate
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor m_type, reset in ImGui::NewFrame(), this updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
    IMGUI_API void          SetMouseCursor(ImGuiMouseCursor m_type);                              // set desired cursor m_type
...
struct ImGuiIO
{
    ...
    int           KeyMap[ImGuiKey_COUNT];   // <unset>              // Map of m_indices into the KeysDown[512] entries array
...
// Types are NOT stored, so it is m_up to you to make sure your Key don't collide with different types.
struct ImGuiStorage
{
    struct Pair
    {
...
struct ImDrawCmd
{
    unsigned int    ElemCount;              // Number of m_indices (multiple of 3) to be rendered as triangles. Vertices are stored in the callee ImDrawList's vtx_buffer[] array, m_indices in idx_buffer[].
    ImVec4          ClipRect;               // Clipping rectangle (x1, y1, x2, y2)
    ImTextureID     TextureId;              // User-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using images or multiple fonts atlas.
    ImDrawCallback  UserCallback;           // If != NULL, call the function instead of rendering the m_vertices. clip_rect and texture_id will be set normally.
...
// The m_type has to be described within the macro (you can either declare the struct or use a typedef)
...
struct ImDrawList
{
    ...
    const char*             _OwnerName;         // Pointer to owner window's m_name (if any) for debugging
    ...
    IMGUI_API void  AddCircle(const ImVec2& m_centre, float m_radius, ImU32 col, int num_segments = 12, float thickness = 1.0f);
    IMGUI_API void  AddCircleFilled(const ImVec2& m_centre, float m_radius, ImU32 col, int num_segments = 12);
    ...
    // Stateful m_path API, add points then finish with PathFill() or PathStroke()
    ...
    IMGUI_API void  PathArcTo(const ImVec2& m_centre, float m_radius, float a_min, float a_max, int num_segments = 10);
    IMGUI_API void  PathArcToFast(const ImVec2& m_centre, float m_radius, int a_min_of_12, int a_max_of_12);                 // Use precomputed angles for a 12 steps circle
...
// NB: If you use a 'glyph_ranges' array you need to make sure that your array persist m_up until the ImFont is cleared. We only copy the pointer, not the data.
struct ImFontAtlas
{
