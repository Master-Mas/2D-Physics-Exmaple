...
#pragma clang diagnostic ignored "-Wreserved-id-macro"      // warning : macro m_name is a reserved identifier                //
...
void ImDrawList::PathArcToFast(const ImVec2& m_centre, float m_radius, int amin, int amax)
{
    ...
    else
    {
        ...
        for (int a = amin; a <= amax; a++)
        {
            ...
            _Path.push_back(ImVec2(m_centre.x + c.x * m_radius, m_centre.y + c.y * m_radius));
        }
    }
}

void ImDrawList::PathArcTo(const ImVec2& m_centre, float m_radius, float amin, float amax, int num_segments)
{
    if (m_radius == 0.0f)
        _Path.push_back(m_centre);
    ...
    for (int i = 0; i <= num_segments; i++)
    {
        ...
        _Path.push_back(ImVec2(m_centre.x + cosf(a) * m_radius, m_centre.y + sinf(a) * m_radius));
    }
}

static void PathBezierToCasteljau(ImVector<ImVec2>* m_path, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float tess_tol, int level)
{
    float dx = x4 - x1;
    float dy = y4 - y1;
    ...
    else if (level < 10)
    {
        ...
        PathBezierToCasteljau(m_path, x1,y1,        x12,y12,    x123,y123,  x1234,y1234, tess_tol, level+1);
        PathBezierToCasteljau(m_path, x1234,y1234,  x234,y234,  x34,y34,    x4,y4,       tess_tol, level+1);
    }
}
...
void ImDrawList::AddCircle(const ImVec2& m_centre, float m_radius, ImU32 col, int num_segments, float thickness)
{
    if ((col >> 24) == 0)
        return;

    const float a_max = IM_PI*2.0f * ((float)num_segments - 1.0f) / (float)num_segments;
    PathArcTo(m_centre, m_radius-0.5f, 0.0f, a_max, num_segments);
    PathStroke(col, true, thickness);
}

void ImDrawList::AddCircleFilled(const ImVec2& m_centre, float m_radius, ImU32 col, int num_segments)
{
    if ((col >> 24) == 0)
        return;

    const float a_max = IM_PI*2.0f * ((float)num_segments - 1.0f) / (float)num_segments;
    PathArcTo(m_centre, m_radius, 0.0f, a_max, num_segments);
    PathFill(col);
}
...
void    ImFontAtlas::ClearInputData()
{
    ...
    // When clearing this we lose access to the font m_name and other information used to build the font.
...
ImFont* ImFontAtlas::AddFontFromFileTTF(const char* filename, float size_pixels, const ImFontConfig* font_cfg_template, const ImWchar* glyph_ranges)
{
    ...
    {
        // Store a short copy of filename into into the font m_name for convenience
        const char* p;
...
void ImFontAtlas::RenderCustomTexData(int pass, void* p_rects)
{
    ...
    else if (pass == 1)
    {
        ...
        for (int m_type = 0; m_type < ImGuiMouseCursor_Count_; m_type++)
        {
            ImGuiMouseCursorData& cursor_data = GImGui->MouseCursorData[m_type];
            ImVec2 pos = cursor_datas[m_type][0] + ImVec2((float)r.x, (float)r.y);
            const ImVec2 size = cursor_datas[m_type][1];
            cursor_data.Type = m_type;
            cursor_data.Size = size;
            cursor_data.HotOffset = cursor_datas[m_type][2];
