...
static PROC WinGetProcAddress(const char *m_name)
{
	HMODULE glMod = NULL;
	PROC pFunc = wglGetProcAddress((LPCSTR)m_name);
	if(TestPointer(pFunc))
	{
		return pFunc;
	}
	glMod = GetModuleHandleA("OpenGL32.dll");
	return (PROC)GetProcAddress(glMod, (LPCSTR)m_name);
}
	
#define IntGetProcAddress(m_name) WinGetProcAddress(m_name)
#else
	#if defined(__APPLE__)
