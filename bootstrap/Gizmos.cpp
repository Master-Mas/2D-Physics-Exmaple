...
// Adds 3 unit-length lines (red,green,blue) representing the 3 axis of a m_transform, 
// at the m_transform's translation. Optional scale available.
void Gizmos::addTransform(const glm::mat4& m_transform, float scale) {

	glm::vec4 vXAxis = m_transform[3] + m_transform[0] * scale;
	glm::vec4 vYAxis = m_transform[3] + m_transform[1] * scale;
	glm::vec4 vZAxis = m_transform[3] + m_transform[2] * scale;

	glm::vec4 vRed(1,0,0,1);
	glm::vec4 vGreen(0,1,0,1);
	glm::vec4 vBlue(0,0,1,1);

	addLine(m_transform[3].xyz(), vXAxis.xyz(), vRed, vRed);
	addLine(m_transform[3].xyz(), vYAxis.xyz(), vGreen, vGreen);
	addLine(m_transform[3].xyz(), vZAxis.xyz(), vBlue, vBlue);
}
...
void Gizmos::addAABB(const glm::vec3& center, 
	...
	if (m_transform != nullptr) {
		vX = (*m_transform * glm::vec4(vX,0)).xyz();
		vY = (*m_transform * glm::vec4(vY,0)).xyz();
		vZ = (*m_transform * glm::vec4(vZ,0)).xyz();
		c = (*m_transform)[3].xyz() + c;
	}
...
void Gizmos::addAABBFilled(const glm::vec3& center, 
	...
	if (m_transform != nullptr) {
		...
		vY = (*m_transform * glm::vec4(vY, 0)).xyz();
		vZ = (*m_transform * glm::vec4(vZ, 0)).xyz();
		tempCenter = (*m_transform)[3].xyz() + tempCenter;
	}
...
void Gizmos::addCylinderFilled(const glm::vec3& center, float m_radius, float fHalfLength,
	unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform) {

	glm::vec4 white(1,1,1,1);

	glm::vec3 tempCenter = m_transform != nullptr ? (*m_transform)[3].xyz() + center : center;

	float segmentSize = (2 * glm::pi<float>()) / segments;

	for ( unsigned int i = 0 ; i < segments ; ++i ) {
		glm::vec3 v0top(0,fHalfLength,0);
		glm::vec3 v1top( sinf( i * segmentSize ) * m_radius, fHalfLength, cosf( i * segmentSize ) * m_radius);
		glm::vec3 v2top( sinf( (i+1) * segmentSize ) * m_radius, fHalfLength, cosf( (i+1) * segmentSize ) * m_radius);
		glm::vec3 v0bottom(0,-fHalfLength,0);
		glm::vec3 v1bottom( sinf( i * segmentSize ) * m_radius, -fHalfLength, cosf( i * segmentSize ) * m_radius);
		glm::vec3 v2bottom( sinf( (i+1) * segmentSize ) * m_radius, -fHalfLength, cosf( (i+1) * segmentSize ) * m_radius);

		if (m_transform != nullptr) {
			v0top = (*m_transform * glm::vec4(v0top, 0)).xyz();
			v1top = (*m_transform * glm::vec4(v1top, 0)).xyz();
			v2top = (*m_transform * glm::vec4(v2top, 0)).xyz();
			v0bottom = (*m_transform * glm::vec4(v0bottom, 0)).xyz();
			v1bottom = (*m_transform * glm::vec4(v1bottom, 0)).xyz();
			v2bottom = (*m_transform * glm::vec4(v2bottom, 0)).xyz();
		}
...
void Gizmos::addRing(const glm::vec3& center, float innerRadius, float outerRadius,
	...
	for ( unsigned int i = 0 ; i < segments ; ++i ) {
		...
		if (m_transform != nullptr) {
			v1outer = (*m_transform * glm::vec4(v1outer, 0)).xyz();
			v2outer = (*m_transform * glm::vec4(v2outer, 0)).xyz();
			v1inner = (*m_transform * glm::vec4(v1inner, 0)).xyz();
			v2inner = (*m_transform * glm::vec4(v2inner, 0)).xyz();
		}
...
void Gizmos::addDisk(const glm::vec3& center, float m_radius,
	unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform) {
	...
	for ( unsigned int i = 0 ; i < segments ; ++i ) {
		glm::vec3 v1outer( sinf( i * fSegmentSize ) * m_radius, 0, cosf( i * fSegmentSize ) * m_radius );
		glm::vec3 v2outer( sinf( (i+1) * fSegmentSize ) * m_radius, 0, cosf( (i+1) * fSegmentSize ) * m_radius );
...
void Gizmos::addArc(const glm::vec3& center, float rotation,
	float m_radius, float arcHalfAngle,
	unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform) {
	...
	for ( unsigned int i = 0 ; i < segments ; ++i ) {
		glm::vec3 v1outer( sinf( i * fSegmentSize - arcHalfAngle + rotation ) * m_radius, 0, cosf( i * fSegmentSize - arcHalfAngle + rotation ) * m_radius);
		glm::vec3 v2outer( sinf( (i+1) * fSegmentSize - arcHalfAngle + rotation ) * m_radius, 0, cosf( (i+1) * fSegmentSize - arcHalfAngle + rotation ) * m_radius);
	...
	// edge lines
	if (fillColour.w == 0) {
		glm::vec3 v1outer( sinf( -arcHalfAngle + rotation ) * m_radius, 0, cosf( -arcHalfAngle + rotation ) * m_radius );
		glm::vec3 v2outer( sinf( arcHalfAngle + rotation ) * m_radius, 0, cosf( arcHalfAngle + rotation ) * m_radius );
...
void Gizmos::addSphere(const glm::vec3& center, float m_radius, int rows, int columns, const glm::vec4& fillColour, 
								const glm::mat4* m_transform, float longMin, float longMax, 
								float latMin, float latMax) {

	float inverseRadius = 1 / m_radius;
	...
	for (int row = 0; row <= rows; ++row) {
		...
		float y  =  m_radius * sin(radiansAboutXAxis);
		float z  =  m_radius * cos(radiansAboutXAxis);
		...
		for ( int col = 0; col <= columns; ++col ) {
			...
			if (m_transform != nullptr) {
				v4Point = (*m_transform * glm::vec4(v4Point, 0)).xyz();
				v4Normal = (*m_transform * glm::vec4(v4Normal, 0)).xyz();
			}
...
void Gizmos::addCapsule(const glm::vec3& center, float height, float m_radius,
						int rows, int cols, const glm::vec4& fillColour, const glm::mat4* rotation) {

	float sphereCenters = (height * 0.5f) - m_radius;
	...
	for (int i = 0; i < cols; ++i) {
		...
		glm::vec4 pos = glm::vec4(cosf(x), 0, sinf(x), 0) * m_radius;
		glm::vec4 pos1 = glm::vec4(cosf(x1), 0, sinf(x1), 0) * m_radius;
...
void Gizmos::add2DAABB(const glm::vec2& center, const glm::vec2& extents, const glm::vec4& colour, const glm::mat4* m_transform /*= nullptr*/) {	
	glm::vec2 verts[4];
	glm::vec2 vX(extents.x, 0);
	glm::vec2 vY(0, extents.y);

	if (m_transform != nullptr) {
		vX = (*m_transform * glm::vec4(vX,0,0)).xy();
		vY = (*m_transform * glm::vec4(vY,0,0)).xy();
	}
...
void Gizmos::add2DAABBFilled(const glm::vec2& center, const glm::vec2& extents, const glm::vec4& colour, const glm::mat4* m_transform /*= nullptr*/) {	
	glm::vec2 verts[4];
	glm::vec2 vX(extents.x, 0);
	glm::vec2 vY(0, extents.y);
...
void Gizmos::add2DCircle(const glm::vec2& center, float m_radius, unsigned int segments, const glm::vec4& colour, const glm::mat4* m_transform /*= nullptr*/) {
	...
	for ( unsigned int i = 0 ; i < segments ; ++i ) {
		glm::vec2 v1outer( sinf( i * segmentSize ) * m_radius, cosf( i * segmentSize ) * m_radius );
		glm::vec2 v2outer( sinf( (i+1) * segmentSize ) * m_radius, cosf( (i+1) * segmentSize ) * m_radius );

		if (m_transform != nullptr) {
			v1outer = (*m_transform * glm::vec4(v1outer,0,0)).xy();
			v2outer = (*m_transform * glm::vec4(v2outer,0,0)).xy();
		}
