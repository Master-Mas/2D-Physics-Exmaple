...
Renderer2D::Renderer2D() {
	...
	// pre calculate the m_indices... they will always be the same
	int index = 0;
...
void Renderer2D::drawCircle(float xPos, float yPos, float m_radius, float depth) {
	...
	for (int i = 0; i < 32; ++i) {
		...
		m_vertices[m_currentVertex].pos[0] = glm::sin(rotDelta * i) * m_radius + xPos;
		m_vertices[m_currentVertex].pos[1] = glm::cos(rotDelta * i) * m_radius + yPos;
		m_vertices[m_currentVertex].pos[2] = depth;
		m_vertices[m_currentVertex].pos[3] = (float)textureID;
		m_vertices[m_currentVertex].color[0] = m_r;
		m_vertices[m_currentVertex].color[1] = m_g;
...
void Renderer2D::drawSpriteTransformed3x3(Texture * texture,
	...
	// m_transform the points by the matrix
	// 0 3 6
	// 1 4 7
	// 2 5 8
	float x, y;
