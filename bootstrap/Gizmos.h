...
public:
	...
	static void		addTransform(const glm::mat4& m_transform, float scale = 1.0f);
	
	// adds a wireframe Axis-Aligned Bounding-Box with optional m_transform for rotation/translation
	static void		addAABB(const glm::vec3& center, const glm::vec3& extents, 
							const glm::vec4& colour, const glm::mat4* m_transform = nullptr);

	// adds an Axis-Aligned Bounding-Box with optional m_transform for rotation
	static void		addAABBFilled(const glm::vec3& center, const glm::vec3& extents, 
								  const glm::vec4& fillColour, const glm::mat4* m_transform = nullptr);

	// adds a cylinder aligned to the Y-axis with optional m_transform for rotation
	static void		addCylinderFilled(const glm::vec3& center, float m_radius, float halfLength,
									  unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform = nullptr);

	// adds a double-sided hollow ring in the XZ axis with optional m_transform for rotation.
	...
	// adds a double-sided disk in the XZ axis with optional m_transform for rotation.
	// if fillColour.w == 0 then only an outer line is drawn
	static void		addDisk(const glm::vec3& center, float m_radius,
							unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform = nullptr);

	// adds an arc, around the Y-axis
	// if fillColour.w == 0 then only an outer line is drawn
	static void		addArc(const glm::vec3& center, float rotation, float m_radius, float halfAngle,
						   unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform = nullptr);

	// adds an arc, around the Y-axis, starting at the inner m_radius and extending to the outer m_radius
	// if fillColour.w == 0 then only an outer line is drawn
	static void		addArcRing(const glm::vec3& center, float rotation, 
							   float innerRadius, float outerRadius, float arcHalfAngle,
							   unsigned int segments, const glm::vec4& fillColour, const glm::mat4* m_transform = nullptr);

	// adds a Sphere at a given position, with a given number of rows, and columns, m_radius and a max and min long and latitude
	static void		addSphere(const glm::vec3& center, float m_radius, int rows, int columns, const glm::vec4& fillColour, 
							  const glm::mat4* m_transform = nullptr, float longMin = 0.f, float longMax = 360, 
							  float latMin = -90, float latMax = 90 );

	// adds a capsule with a set height and m_radius
	static void		addCapsule(const glm::vec3& center, float height, float m_radius,
	...
	static void		add2DAABB(const glm::vec2& center, const glm::vec2& extents, const glm::vec4& colour, const glm::mat4* m_transform = nullptr);	
	static void		add2DAABBFilled(const glm::vec2& center, const glm::vec2& extents, const glm::vec4& colour, const glm::mat4* m_transform = nullptr);	
	static void		add2DCircle(const glm::vec2& center, float m_radius, unsigned int segments, const glm::vec4& colour, const glm::mat4* m_transform = nullptr);
